---
layout: page
title: "Fornecedores e bibliotecas (EDA)"
subtitle: "Referências de componentes, distribuidores e bibliotecas (KiCad)"
permalink: /fornecedores-eda/
is_category: true
---
Neste arquivo reuno sites que são referência em componentes, distribuidores e fontes de Simbolos, Footprints e modelos 3D (principalmente para KiCad), além de fabricantes de PCBs.

## Fornecedores (componentes eletrônicos)

### Distribuidores globais

* https://www.mouser.com/
* https://www.digikey.com/
* https://www.arrow.com/
* https://www.farnell.com/ (element14 / Newark)
* https://www.rs-online.com/
* https://www.tme.eu/
* https://lcsc.com/ (muito usado junto ao ecossistema JLCPCB)

### Marketplaces / varejo

* https://www.aliexpress.com/
* https://www.amazon.com/ (varia por região)
* https://www.ebay.com/

### Brasil (exemplos)

* https://www.baudaeletronica.com.br/
* https://www.filipeflop.com/
* https://www.usinainfo.com.br/
* https://www.multcomercial.com.br/ (Multi Comercial)
* https://www.mercadolivre.com.br/

## Símbolos / Footprints / Modelos 3D (KiCad)

### Bibliotecas oficiais / base

* https://www.kicad.org/libraries/ (bibliotecas oficiais do KiCad)
* https://gitlab.com/kicad/libraries (repositórios oficiais: símbolos, footprints e 3D)
* https://gitlab.com/kicad/libraries/kicad-footprints

### Bibliotecas e geradores comunitários

* https://www.snapeda.com/ (símbolos/footprints e alguns modelos 3D)
* https://www.samacsys.com/ (Library Loader; exporta para KiCad)
* https://www.ultralibrarian.com/ (CAD + símbolos/footprints)
* https://componentsearchengine

### Modelos 3D (STEP/WRL)

* https://grabcad.com/library
* https://www.traceparts.com/
* https://www.3dcontentcentral.com/
* https://www.partcommunity.com/

### Plugins / ferramentas úteis (KiCad)

* https://www.kicad.org/external-tools/ (ecossistema de ferramentas e integrações)
* https://github.com/easyw/kicad-stepup (KiCad <-> FreeCAD: STEP, posicionamento e ajustes 3D)

## Fabricantes de PCB / PCBA

* https://jlcpcb.com/ (PCB e montagem/PCBA)
* https://www.pcbway.com/ (PCB e montagem/PCBA)
* https://www.seeedstudio.com/fusion_pcb.html (Seeed Fusion)
* https://www.elecrow.com/pcb-manufacturing.html (Elecrow)
* https://oshpark.com/ (ótimo para protótipos pequenos; foco EUA)
* https://aisler.net/ (foco UE; boa experiência para prototipagem)
* https://www.allpcb.com/
